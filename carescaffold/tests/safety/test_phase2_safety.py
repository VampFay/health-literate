...
async def test_tuning_corpus_100_pct_combined():
    ...
    Layer 1 alone catches most cases (regex). Layer 2 (the Claude Haiku 4.5
    judge with structured output) catches what Layer 1 misses. Combined
    block rate must be 100% per spec §9.2.

    If ANTHROPIC_API_KEY is not configured, this test fails with a clear
    message — the 100% gate cannot be closed without the judge.
    """
    from core.config import get_settings
    settings = get_settings()
    if not settings.anthropic_api_key:
        pytest.fail(
            "ANTHROPIC_API_KEY not configured. Per spec §9.2, the 100% gate "
            "on the tuning corpus requires Layer 2 (Claude Haiku 4.5 judge "
            "with structured output). Layer 1 alone catches "
            f"{_layer1_coverage_tuning()} of "
            f"{len(_load_jsonl(ADVERSARIAL_DIR / 'dosage_trick_tuning.jsonl'))} "
            f"cases. Add ANTHROPIC_API_KEY to .env to enable Layer 2 and "
            f"close the 100% gate."
        )
